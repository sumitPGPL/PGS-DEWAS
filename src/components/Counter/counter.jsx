...
const CounterWidget = ({ initialValue = 0, targetValue = 10, speed = 50, textColor = 'black', bgColor = 'white', title = 'Counter Widget' }) => {
  const count = useAutoIncrement(initialValue, targetValue, speed);

  return (
    <div className={`p-4 border border-gray-300 rounded-md text-center text-lg ${bgColor} ${textColor}`}>
      <h2>{title}</h2>
      <p>Count: {count}</p>
    </div>
  );
