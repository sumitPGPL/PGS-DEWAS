...
const Monuments = () => {
    ...
    return (
        <div className="w-full lg:w-10/12 md:w-10/12 mx-auto mt-8 p-4 bg-white rounded-lg shadow-md">
            <div className="flex justify-between overflow-scroll">
                {Object.keys(monumentsData).map((monument) => (
                    <button
                        ...
                        className={`p-2 text-sm focus:outline-none ${selectedMonument === monument ? 'rounded-t-lg font-bold bg-[#92cee7] p-5' : 'font-normal'}`}
                    >
            ...
            <div >
                <p className="text-gray-700 bg-[#92cee7] p-5 rounded-b-lg">{monumentsData[selectedMonument].content}</p>
            </div>
        </div>
    );
};
