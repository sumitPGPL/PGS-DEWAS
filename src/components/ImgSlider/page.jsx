...
export default function page({images}) {
  ...
  return (
    ...
    <div className="flex justify-center items-center">
        <div className="flex max-w-full overflow-x-scroll scroll-smooth" ref={sliderRef}>
          {images.map((image) => {
            return (
              <Image
                width={300}
      ...
      <div className="flex justify-center mt-4">
        <button onClick={scrollLeft} className="px-4 py-2 mr-2 shadow rounded-md">
        <Image alt="image" src={'/left-arrow.svg'} width={30} height={30}></Image>
        </button>
        <button onClick={scrollRight} className="px-4 py-2 shadow rounded-md">
          <Image alt="image" src={'/right-arrow.svg'}  width={30} height={30}></Image>
        </button>
      </div>
    </>
  );
